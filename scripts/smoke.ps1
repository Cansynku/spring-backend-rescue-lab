...
$paymentResult = Invoke-WebRequest -Method Post -Uri "$BaseUrl/api/orders/$($order.id)/payments" -Headers @{'Idempotency-Key'=$paymentKey} -ContentType 'application/json' -Body '{"amount":25.50}'
...
$replayResult = Invoke-WebRequest -Method Post -Uri "$BaseUrl/api/orders/$($order.id)/payments" -Headers @{'Idempotency-Key'=$paymentKey} -ContentType 'application/json' -Body '{"amount":25.50}'
$replay = $replayResult.Content | ConvertFrom-Json
if ($replayResult.StatusCode -ne 200 -or $replay.id -ne $payment.id) { throw 'Replay did not reuse the original payment' }
...
[pscustomobject]@{
    ...
    ReplayPayment = $replayResult.StatusCode
